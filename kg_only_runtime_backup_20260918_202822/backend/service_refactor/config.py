...
class Step9RuntimeConfig:
    ...
    def from_env(cls) -> "Step9RuntimeConfig":
        ...
        graph_only_guard = (
            os.getenv("KG_LLM_GRAPH_ONLY", "false").strip().lower()
            in {"1", "true", "yes", "y", "on"}
            and os.getenv("KG_RUNTIME_DATA_SOURCE", "legacy").strip().lower()
            == "knowledge_graph"
        )
        if graph_only_guard:
            # Explicit KG-only mode is a stronger safety contract than the old
            # Step-9 default. It prevents accidental legacy fallback even if a
            # deployment forgot to change STEP9_RUNTIME_MODE.
            mode = RuntimeMode.GRAPH_ONLY
        else:
            try:
        ...
        if mode == RuntimeMode.GRAPH_ONLY or graph_only_guard:
