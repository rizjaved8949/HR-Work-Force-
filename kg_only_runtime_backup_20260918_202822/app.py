...
install_kg_runtime_source_headers(app)
app.include_router(auth_router)
app.include_router(create_step9_runtime_router(step9_runtime))
app.include_router(kg_runtime_router)
...
def run_employee_search(
    ...
    result = employee_search_tool.invoke({
        "employee_id": employee_id,
        "employee_name": employee_name,
        "department": department,
    })
    if isinstance(result, dict):
        result.setdefault("runtime_source", runtime_source_metadata())
    return result
...
def predict_from_employee_record(request: AttritionRecordRequest):
    ...
    result = attrition_prediction_tool.invoke({
