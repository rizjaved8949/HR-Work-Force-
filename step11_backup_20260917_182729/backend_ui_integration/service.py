...
class ExistingHRUIIntegrationService:
    ...
    def bootstrap(self, user: Any = None) -> UIIntegrationBootstrap:
        runtime_status = self.runtime.status()
        context = self.user_context(user)
        warnings = list(runtime_status.warnings)
        if not runtime_status.graph_available:
            warnings.append(
                "Knowledge Graph is not active for this process; compatible UI routes may be using legacy fallback."
            )
        return UIIntegrationBootstrap(
            app_name=self.config.app_name,
            tenant_id=runtime_status.tenant_id,
            ...
            metadata={
                ...
                "multi_org_switching": False,
                "multi_org_note": "Organization switching belongs to roadmap Step 12; Step 11 intentionally uses the current Step-9 tenant.",
