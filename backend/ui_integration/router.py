...
def create_ui_integration_router(
    ...
    def navigation(request: Request):
        return {
            "tenant_id": getattr(request.state, "tenant_id", runtime.config.tenant_id),
    ...
    def runtime_status(request: Request):
        selected_tenant = getattr(request.state, "tenant_id", runtime.config.tenant_id)
        return {
            "step": 11,
