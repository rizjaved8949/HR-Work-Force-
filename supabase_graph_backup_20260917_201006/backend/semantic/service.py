...
_TEMPORAL_PROPERTIES: dict[str, tuple[str, ...]] = {
    ...
    "PositionBudget": ("dataAsOfDate", "budgetStartDate", "budgetEndDate"),
    "VacancyRecord": ("dataAsOfDate", "vacancyStartDate", "vacancyEndDate"),
}
...
class SemanticHRService:
    ...
    def get_organizational_unit(self, *, tenant_id: str, employee_id: str) -> SemanticRecord | None:
        return self._assignment_related(
            tenant_id=tenant_id, employee_id=employee_id,
            relation_type="IN_ORGANIZATIONAL_UNIT", entity_type="OrganizationalUnit"
        )

    def get_work_location(self, *, tenant_id: str, employee_id: str) -> SemanticRecord | None:
        return self._assignment_related(
            tenant_id=tenant_id, employee_id=employee_id,
            relation_type="AT_LOCATION", entity_type="WorkLocation"
        )

    def get_cost_center(self, *, tenant_id: str, employee_id: str) -> SemanticRecord | None:
        return self._assignment_related(
            tenant_id=tenant_id, employee_id=employee_id,
            relation_type="CHARGED_TO", entity_type="CostCenter"
        )

    def get_business_unit(self, *, tenant_id: str, employee_id: str) -> SemanticRecord | None:
        department = self.get_department(tenant_id=tenant_id, employee_id=employee_id)
        if department is None:
            return None
        nodes = self.repository.related_nodes(
            tenant_id=tenant_id, graph_id=department.reference_id,
            relation_type="HAS_DEPARTMENT", direction="in",
            entity_type="BusinessUnit", limit=2,
        )
        if len(nodes) > 1:
            raise SemanticDataIntegrityError(
                f"Department {department.reference_id!r} belongs to multiple BusinessUnit nodes"
            )
        return SemanticRecord.from_node(nodes[0]) if nodes else None

    def _position_related(
        self,
        *,
        tenant_id: str,
        employee_id: str,
        ...
        position = self._assignment_related(
            tenant_id=tenant_id,
            employee_id=employee_id,
            relation_type="TO_POSITION",
            entity_type="Position",
        ...
        nodes = self.repository.related_nodes(
            tenant_id=tenant_id,
            graph_id=position.reference_id,
            relation_type=relation_type,
            direction="out",
            entity_type=entity_type,
            limit=500 if latest else 2,
        )
        if not nodes:
            return None
        if latest:
            selected = _latest(nodes)
            return SemanticRecord.from_node(selected) if selected else None
        if len(nodes) > 1:
            raise SemanticDataIntegrityError(
                f"Current position has multiple {relation_type} targets for tenant {tenant_id!r}"
            )
        return SemanticRecord.from_node(nodes[0])

    def get_reporting_position(
        self, *, tenant_id: str, employee_id: str
    ) -> SemanticRecord | None:
    ...
    def get_position_budget(
        ...
        return self._position_related(
            ...
            entity_type="PositionBudget",
            latest=True,
        )

    def get_position_vacancy(
        self, *, tenant_id: str, employee_id: str
    ) -> SemanticRecord | None:
        return self._position_related(
            tenant_id=tenant_id,
            employee_id=employee_id,
            relation_type="HAS_VACANCY",
            entity_type="VacancyRecord",
            latest=True,
        )

    def get_position_skill_requirement_skill(
        self, *, tenant_id: str, requirement_reference_id: str
    ) -> SemanticRecord | None:
        nodes = self.repository.related_nodes(
            tenant_id=tenant_id,
            graph_id=requirement_reference_id,
        ...
        if len(nodes) > 1:
            raise SemanticDataIntegrityError(
                f"Position skill requirement {requirement_reference_id!r} maps to multiple Skill nodes"
            )
        return SemanticRecord.from_node(nodes[0]) if nodes else None

    def get_position_requirements(
        self, *, tenant_id: str, employee_id: str
    ) -> list[SemanticRecord]:
        position = self._assignment_related(
            tenant_id=tenant_id, employee_id=employee_id,
            relation_type="TO_POSITION", entity_type="Position"
        )
        if position is None:
            return []
        nodes = self.repository.related_nodes(
            tenant_id=tenant_id, graph_id=position.reference_id,
            relation_type="HAS_REQUIREMENT", direction="out",
            entity_type="PositionRequirement", limit=100,
        )
        return [SemanticRecord.from_node(node) for node in sorted(nodes, key=_node_rank, reverse=True)]

    def get_position_skill_requirements(
        self, *, tenant_id: str, employee_id: str
    ) -> list[SemanticRecord]:
        position = self._assignment_related(
            tenant_id=tenant_id, employee_id=employee_id,
        ...
        nodes = self.repository.related_nodes(
            tenant_id=tenant_id, graph_id=position.reference_id,
            relation_type="REQUIRES_SKILL", direction="out",
            entity_type="PositionSkillRequirement", limit=500,
        )
        return [SemanticRecord.from_node(node) for node in sorted(nodes, key=_node_rank, reverse=True)]
    ...
    def get_employee_context(
        ...
        return EmployeeContext(
            ...
            department=self.get_department(tenant_id=tenant_id, employee_id=employee_id),
            business_unit=self.get_business_unit(tenant_id=tenant_id, employee_id=employee_id),
            organizational_unit=self.get_organizational_unit(tenant_id=tenant_id, employee_id=employee_id),
            work_location=self.get_work_location(tenant_id=tenant_id, employee_id=employee_id),
            cost_center=self.get_cost_center(tenant_id=tenant_id, employee_id=employee_id),
            position=self.get_position(tenant_id=tenant_id, employee_id=employee_id),
            reporting_position=self.get_reporting_position(tenant_id=tenant_id, employee_id=employee_id),
            position_budget=self.get_position_budget(tenant_id=tenant_id, employee_id=employee_id),
            vacancy=self.get_position_vacancy(tenant_id=tenant_id, employee_id=employee_id),
            manager=self.get_manager(tenant_id=tenant_id, employee_id=employee_id),
            position_requirements=self.get_position_requirements(tenant_id=tenant_id, employee_id=employee_id),
            position_skill_requirements=self.get_position_skill_requirements(tenant_id=tenant_id, employee_id=employee_id),
